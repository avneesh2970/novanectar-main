...
const ServiceCard = React.memo(({ title, description, icon }: any) => {
  return (
    <motion.div
      initial="initial"
      whileHover="hover"
...
export default function ProcessSection() {
  ...
  // Use a ref instead of state for isMobile to avoid re-renders
  ...
  const x = useTransform(scrollYProgress, [0, 1], ["0%", "-1850%"], { clamp: true })

  // Use a more performant spring configuration
  const xSpring = useSpring(x, {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001, // More efficient stopping threshold
  })
  ...
  return (
    <div ref={containerRef} className={`relative w-full ${isMobile ? "h-[33rem]" : "h-[200vh]"} bg-gray-900`}>
      <div
        className={`${
          isMobile ? "pt-6" : "sticky top-0"
        } h-screen flex flex-col items-start justify-start sm:justify-center overflow-hidden`}
      >
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8">
          ...
          >
           ...
           {/* <h2
          className={`text-3xl sm:text-4xl md:text-5xl text-center font-semibold text-white mb-14 ${DMSans.className}`}
        >
          ...
          <div className={`overflow-hidden ${isMobile ? "overflow-x-auto" : ""}`}>
            <motion.div style={isMobile ? {} : { x: xSpring }} className="flex space-x-4 sm:space-x-6 lg:space-x-8">
              {serviceCards}
            </motion.div>
          </div>
        </div>
