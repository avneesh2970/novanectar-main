...
const ProjectCard = ({ item, index }: any) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="p-6 sm:p-8 lg:p-12 h-96 lg:h-[28rem] flex items-center justify-center relative overflow-hidden">
          <motion.div
      ...
      {/* Text section */}
      <div className="space-y-1.5 sm:space-y-2 px-4 sm:px-6 pb-6">
        <h3 className="text-gray-700 text-lg sm:text-xl font-semibold leading-tight">
          {item.title}
        </h3>
        ...
        <div className="flex flex-wrap items-center">
          {item.tags.map((tag: any, tagIndex: any) => (
            ...
            >
              {tagIndex !== 0 && (
                <span className="text-black/50 pr-1 text-lg sm:text-xl">•</span>
              )}
              {tag}
            </span>
          ))}
...
const ProjectSession = () => {
  ...
  return (
    <section
      className={`${DMSans400.className} bg-[#fff2f2] px-4 py-16 sm:px-6 lg:px-8`}
    >
      <div className="max-w-[1400px] mx-auto">
        {/* Section heading */}
        <motion.div
        ...
        >
          <h2
            className={`text-3xl sm:text-4xl md:text-5xl font-semibold text-gray-800 mb-14 ${DMSans.className}`}
          >
            <span className="inline-block relative">
              Projects
              <span className="absolute -bottom-2 left-0 w-full h-1 bg-[#3DBBFA]"></span>
